...
export default async function ControlListingsPage({
  ...
  const { staff } = await requireControlPermission("manage_catalog", "/control/listings");
  const params = (await searchParams) ?? {};
  const query = params.q?.trim().toLowerCase() ?? "";
  const status =
    params.status === "live" ? "live" : params.status === "hidden" ? "hidden" : "all";
  ...
  if (productsResult.error) throw new Error(`Listing product lookup failed: ${productsResult.error.message}`);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <MetricCard label="Published" value={String(activeListings)} detail="Listing publication flag enabled" />
        <MetricCard label="Configurations" value={String(configurations.length)} detail="Storefront configuration records" />
      </section>
            ...
            {products.map((product) => {
              ...
              return (
                <Link
                  ...
                  href={`/control/listings/${product.id}`}
                  key={product.id}
                >
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {configurations.map((configuration) => (
            <Link
              className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-emerald-500 hover:shadow-md"
              href={`/control/listings/configurations/${encodeURIComponent(configuration.key)}`}
