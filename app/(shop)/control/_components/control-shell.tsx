  ...
  return (
    <div className="min-h-screen min-w-0 lg:grid lg:grid-cols-[17rem_minmax(0,1fr)]">
      <aside className="border-b border-zinc-800 bg-zinc-950 text-zinc-100 lg:min-h-screen lg:border-b-0 lg:border-r">
        <div className="flex min-h-16 items-center justify-between gap-3 px-4 sm:px-5 lg:block lg:py-5">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-400">
              Control
            </p>
            <p className="mt-1 hidden truncate text-base font-semibold sm:block sm:text-lg">
              Operations console
            </p>
          </div>
          <div className="flex shrink-0 items-center gap-2 lg:mt-3">
            <span className="hidden rounded-full border border-zinc-700 px-2.5 py-1 text-xs font-medium capitalize text-zinc-300 sm:inline-flex">
              {staff.role}
            </span>
            <ControlMobileNavigation links={mobileLinks} role={staff.role} />
          </div>
        </div>
