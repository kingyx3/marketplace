...
import {
  SupplierForm,
  type SupplierRecord,
} from "@/app/(shop)/control/_components/supplier-form";
...
export default async function SupplierDetailPage({
  ...
  await requireControlPermission("manage_suppliers", `/control/suppliers/${supplierId}`);
    ...
    <div className="space-y-8">
      <PageHeader
        action={
          <>
            ...
            <ControlBackLink href="/control/suppliers">Back to suppliers</ControlBackLink>
          </>
        }
      ...
      <section className="rounded-xl border border-rose-100 bg-white p-5 shadow-sm">
        ...
        <form action={setControlSupplierActive} className="mt-4">
          ...
          <ControlDangerButton>{supplier.active ? "Archive supplier" : "Restore supplier"}</ControlDangerButton>
        </form>
      </section>
    </div>
  );
