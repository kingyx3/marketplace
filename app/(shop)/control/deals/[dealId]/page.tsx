...
export default async function DealDetailPage({
  ...
  await requireControlPermission("manage_catalog", `/control/deals/${dealId}`);
    ...
    <div className="space-y-8">
      <PageHeader
        action={
          <>
            ...
            <ControlBackLink href="/control/deals">Back to deals</ControlBackLink>
          </>
        }
      ...
      <section className="rounded-xl border border-rose-100 bg-white p-5 shadow-sm">
        ...
        <form action={setLimitedTimeDealActive} className="mt-4">
          ...
          <ControlDangerButton>{deal.active ? "Deactivate deal" : "Activate deal"}</ControlDangerButton>
        </form>
      </section>
    </div>
  );
