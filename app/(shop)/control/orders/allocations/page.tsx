...
export default async function ControlPreorderAllocationPage({
  ...
  if (selectedSkuId) {
    try {
      ...
      previewError =
        error instanceof Error ? error.message : "Allocation preview could not be loaded";
    }
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600">
              Only fully paid retail preorders appear here. Normal orders are never allocated
      ...
      >
        ...
        <label className="flex items-start gap-3 text-sm text-amber-950">
          ...
          <span>
            ...
            and sends Stripe refunds for every unallocated unit. The preview is rejected if stock or
            the queue changes before submission.
          </span>
        </label>
...
function ErrorMessage({ code }: { code: string }) {
  const messages: Record<string, string> = {
    ...
    "refund-failed":
      "Stripe did not confirm every required refund. The staged allocation remains retryable; submit the same queue again after reviewing Stripe.",
    "payment-missing":
      "A preorder is missing its captured full-payment record and cannot be allocated.",
    "allocation-failed":
      "The allocation could not be completed. No unstaged allocation was applied.",
  };
  return (
    <div className="rounded-md border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">
