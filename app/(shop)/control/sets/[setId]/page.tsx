...
import { ControlBackLink, ControlDangerButton, ControlData } from "@/app/(shop)/control/_components/control-resource-ui";
import {
  SetForm,
  type CategoryOption,
  type SetDraft,
...
export default async function SetDetailPage({
  ...
  await requireControlPermission("manage_catalog", `/control/sets/${setId}`);
  ...
  if (productCountResult.error) throw new Error(`Set product count failed: ${productCountResult.error.message}`);
  if (!setResult.data) notFound();

  const set = setResult.data as SetRecord;
  const categories = (categoryResult.data ?? []) as CategoryOption[];
  const categoryName = categories.find((category) => category.id === set.category_id)?.name ?? "Unknown category";
    ...
    <div className="space-y-8">
      <PageHeader
        action={
          <>
            ...
            <ControlBackLink href="/control/sets">Back to sets</ControlBackLink>
          </>
        }
      ...
      <section className="rounded-xl border border-rose-100 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          Sets with active products cannot be archived.
        </p>
