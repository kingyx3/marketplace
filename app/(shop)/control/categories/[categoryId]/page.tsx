...
export default async function CategoryDetailPage({
  ...
  await requireControlPermission("manage_catalog", `/control/categories/${categoryId}`);
  ...
  if (categoriesResult.error) throw new Error(`Category options failed: ${categoriesResult.error.message}`);
  if (productCountResult.error) throw new Error(`Category product count failed: ${productCountResult.error.message}`);
  if (setCountResult.error) throw new Error(`Category set count failed: ${setCountResult.error.message}`);
    ...
    <div className="space-y-8">
      <PageHeader
        action={
          <>
            ...
            <ControlBackLink href="/control/categories">Back to categories</ControlBackLink>
          </>
        }
      ...
      <section className="rounded-xl border border-rose-100 bg-white p-5 shadow-sm">
        ...
        <form action={setControlCategoryActive} className="mt-4">
          ...
          <ControlDangerButton>{category.active ? "Archive category" : "Restore category"}</ControlDangerButton>
        </form>
      </section>
    </div>
  );
