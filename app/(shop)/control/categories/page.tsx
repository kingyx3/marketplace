...
export default async function ControlCategoriesPage({
  ...
  const { staff } = await requireControlPermission("manage_catalog", "/control/categories");
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        action={
          <>
            ...
            <ControlPrimaryLink href="/control/categories/new">Add category</ControlPrimaryLink>
          </>
        }
      ...
      {categories.length === 0 ? (
        <ControlEmptyState
          action={<ControlPrimaryLink href="/control/categories/new">Add category</ControlPrimaryLink>}
            ...
            {categories.map((category) => (
              <article key={category.id} className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <Link
                        ...
                        href={`/control/categories/${category.id}`}
                      >
                        {category.name}
                      </Link>
                ...
                <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
                  ...
                  <ControlData label="Products" value={String(productCounts.get(category.id) ?? 0)} />
                </dl>
              </article>
            ))}
          </div>
