...
UI visibility is not treated as authorization. Privileged database mutations use service-role-only RPCs after the server has verified both the email allowlist and an active staff row. Deal audience rules are also enforced by database row-level security so hidden member metadata cannot be fetched through an anonymous client.
...
Production environment validation requires `SUPPORT_EMAIL` and `ADMIN_EMAIL_ALLOWLIST`. Use a monitored support address, and store the comma-separated admin emails in the repository variable without exposing it to browser code.

## Owner release checks

Before production launch, the operator must:

1. Set `APP_NAME` to the actual contracting operator or approved trading name, publish the correct `SUPPORT_EMAIL`, and configure `ADMIN_EMAIL_ALLOWLIST`.
