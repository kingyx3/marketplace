...
from `GCP_PROJECT_ID` and the project slug unless `TF_STATE_BUCKET_NAME` is set.
...
Optional repository variables are `PROJECT_SLUG`, `TF_STATE_BUCKET_NAME`,
`TF_STATE_BUCKET_LOCATION`, `VERCEL_PROJECT_NAME`, `VERCEL_TEAM_ID`,
`VERCEL_ROOT_DIRECTORY`, `SUPABASE_REGION`, and `SUPABASE_INSTANCE_SIZE`.
Defaults are derived in the workflows or Terraform.
...
1. Set the required CI/CD variables and secrets listed above.
