...
for (const name of environmentVariables) {
  ...
  else if (name === "NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE") setVariable(target, name, "0.1");
  else if (name === "NEXT_PUBLIC_SENTRY_REPLAYS_SESSION_SAMPLE_RATE") setVariable(target, name, "0.02");
  else if (name === "NEXT_PUBLIC_SENTRY_REPLAYS_ON_ERROR_SAMPLE_RATE") setVariable(target, name, "1");
...
function secretIsRequired(name) {
  ...
  if (name === "SENTRY_AUTH_TOKEN") return target === "production";
  if (
    target === "development" &&
    [
      "CRON_SECRET",
