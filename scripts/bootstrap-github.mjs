...
const targetVariables = {
  development: [],
  staging: ["RECOVERY_PROJECT_REF", "RESTORE_RTO_SECONDS", ...hostedOperationsVariables],
  production: [
...
for (const name of environmentSecrets) {
  reportInput(
    `${environmentPrefix(target)}_${name}`,
    environmentValue(target, name),
    secretIsRequired(name)
  );
}
if (target === "production") {
  reportInput("PRODUCTION_REVIEWERS", process.env.PRODUCTION_REVIEWERS, true);
}
...
for (const [name, value] of Object.entries(sharedVariableValues)) {
  setRepositoryVariable(name, value);
}
for (const name of sharedSentryVariables) {
...
for (const name of sharedSentrySecrets) {
  ...
  if (value) setSecret(name, value);
    ...
    fail(`${name} is required for hosted Sentry source maps`);
  }
}
for (const name of sharedRuntimeVariables) {
  const value = process.env[name] || "";
  if (value) setRepositoryVariable(name, value);
    ...
    fail(`${name} is required for production admin authorization`);
  }
}
...
function secretIsRequired(name) {
  if (["SUPABASE_SECRET_KEY", "GOOGLE_OAUTH_CLIENT_SECRET"].includes(name)) return false;
  if (
    target === "development" &&
    [
      "CRON_SECRET",
...
function ensureEnvironment(environment) {
  ...
  const payload = {
    ...
    prevent_self_review:
      environment === "production" ? true : Boolean(current?.prevent_self_review),
    reviewers,
...
function ensureDeploymentPolicy(environment, pattern) {
  ...
  runJson("gh", ["api", "--method", "POST", endpoint, "--input", "-"], {
    name: pattern,
  });
...
function currentWaitTimer(environment) {
  const rule = (environment?.protection_rules || []).find(
    (candidate) => candidate.type === "wait_timer"
  );
...
function run(command, commandArgs, options = {}) {
  ...
  if (result.error || result.status !== 0) {
    fail(
      `${command} ${commandArgs.join(" ")} failed${result.stderr ? `: ${result.stderr.trim()}` : ""}`
    );
  }
  return result;
}
