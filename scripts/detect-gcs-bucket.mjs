...
const credentials = parseCredentials(
  process.env.GCP_TERRAFORM_CREDENTIALS_JSON || process.env.GOOGLE_CREDENTIALS || ""
);
...
if (process.env.GITHUB_ENV)
  await appendFile(process.env.GITHUB_ENV, `TF_STATE_BUCKET_EXISTS=${value}\n`, "utf8");
if (process.env.GITHUB_OUTPUT)
  await appendFile(process.env.GITHUB_OUTPUT, `exists=${value}\n`, "utf8");
...
async function accessToken(credentials) {
  ...
  if (!response.ok)
    fail(
      `Google OAuth token exchange failed with HTTP ${response.status}: ${await response.text()}`
    );
