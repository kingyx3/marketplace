...
const deploymentKey = createHash("sha256").update(`${target}\n${revision}\n${configFingerprint}`).digest("hex");
...
const args = ["--yes", pinnedNpxPackage("vercel"), "deploy", "--yes", "--token", token,
  "--meta", `marketplaceDeploymentKey=${deploymentKey}`, "--meta", `githubCommitSha=${revision}`];
if (targetEnv === "production") args.push("--prod");
const result = spawnSync("npx", args, { encoding: "utf8", env: process.env, stdio: ["ignore", "pipe", "pipe"] });
...
if (!deploymentUrl || !/^https?:\/\//.test(deploymentUrl)) fail("Vercel deploy did not return a deployment URL");
await exportDeploymentUrl(deploymentUrl);
...
async function findReadyDeployment(key) {
  ...
  if (!response.ok) fail(`Could not list Vercel deployments (${response.status}): ${await response.text()}`);
...
async function exportDeploymentUrl(deploymentUrl) {
  ...
  if (process.env.GITHUB_OUTPUT) await appendFile(process.env.GITHUB_OUTPUT, `deployment_url=${deploymentUrl}\n`, "utf8");
}
