...
async function printPlan(config) {
  const current =
    config.accessToken && config.projectRef ? await fetchHostedAuthConfig(config) : null;
    ...
    inspect(
      {
        ...
        supabaseHostedProject: {
          ...
          changes: current
            ? diffAuthConfig(current, desired)
            : "Management credentials unavailable; desired state only",
        },
      },
...
async function applyHostedProvider(config, { skipWhenMissing }) {
  ...
  if (Object.keys(patch).length === 0) {
    console.log(
      `Supabase hosted auth configuration is already converged for ${config.projectRef}.`
    );
    return;
  }
  ...
  if (!response.ok)
    throw new Error(
      `Supabase Auth configuration update failed (${response.status}): ${redact(await response.text())}`
    );
...
async function verifyHostedProvider(config, { strict }) {
  ...
  if (!config.supabaseUrl || !config.publishableKey) {
    const message =
      "Cannot verify Google OAuth provider without Management API credentials or public Supabase settings.";
    if (strict) throw new Error(message);
    console.log(message);
    return;
  }
  ...
  if (enabled !== config.enabled && strict)
    throw new Error(`Supabase external.google=${enabled}; expected ${config.enabled}.`);
  console.log(
    `Supabase Auth Google provider reported by /auth/v1/settings: ${enabled ? "enabled" : "disabled"}`
  );
}
...
async function fetchHostedAuthConfig(config) {
  ...
  if (!response.ok)
    throw new Error(
      `Supabase Auth configuration lookup failed (${response.status}): ${redact(await response.text())}`
    );
...
function diffAuthConfig(current, desired) {
  ...
  for (const [key, desiredValue] of Object.entries(desired)) {
    if (key === "external_google_secret") {
      const clientChanged =
        current?.external_google_client_id !== desired.external_google_client_id;
      const enabling = !current?.external_google_enabled && desired.external_google_enabled;
      const missing =
        current?.external_google_secret === "" || current?.external_google_secret === null;
      if (clientChanged || enabling || missing) patch[key] = desiredValue;
      continue;
    }
    const currentValue =
      key === "uri_allow_list" ? normalizeAllowList(current?.[key]) : current?.[key];
    const normalizedDesired =
      key === "uri_allow_list" ? normalizeAllowList(desiredValue) : desiredValue;
...
function sanitizeDesired(desired) {
  return {
    ...desired,
    ...(desired.external_google_secret ? { external_google_secret: "[configured]" } : {}),
  };
}
