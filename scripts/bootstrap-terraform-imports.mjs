...
const supabaseEnvironments = (
  optional("TERRAFORM_BOOTSTRAP_SUPABASE_ENVIRONMENTS") || "development,production"
)
  .split(",")
  .map((env) => env.trim())
  .filter(Boolean);
...
async function bootstrapVercelProjects() {
  const primaryProjectName =
    firstPresent("TF_VAR_vercel_project_name", "VERCEL_PROJECT_NAME") ||
    firstPresent("TF_VAR_project_slug", "PROJECT_SLUG");
  if (!primaryProjectName)
    fail("Project slug/name is required to reconcile the Vercel Terraform state.");
...
async function bootstrapVercelProject(address, projectName) {
  ...
  if (!project) {
    console.log(
      `No existing Vercel project named ${projectName} was found; Terraform may create it.`
    );
    return;
  }
  ...
  console.log(
    `Importing existing Vercel project ${projectName} (${projectId}) into Terraform state.`
  );
...
async function bootstrapSupabaseProjects() {
  ...
  console.log(
    `Supabase state reconciliation can see ${projects.length} project(s): ${visibleSupabaseProjectNames(projects)}`
  );
  ...
  for (const env of supabaseEnvironments) {
    ...
    if (state.exists) {
      if (!state.id)
        fail(`${address} is in Terraform state but its project ref could not be read.`);
      if (visibleProjectRefs.has(state.id)) {
        console.log(`${address} (${state.id}) still exists in Supabase; skipping import.`);
        continue;
      }
      console.log(
        `${address} points to deleted Supabase project ${state.id}; removing only the stale state entry.`
      );
    ...
    if (!project) {
      console.log(
        `No existing Supabase project for ${env} matched ${projectNames.join(", ")}; Terraform may create it.`
      );
      continue;
    }
    ...
    if (!projectRef)
      fail(
        `Supabase project ${supabaseProjectDisplayName(project)} response did not include an id/ref.`
      );
    console.log(
      `Importing existing Supabase project ${supabaseProjectDisplayName(project)} (${projectRef}) into Terraform state.`
    );
...
function selectSupabaseProject(projects, projectNames, organizationId, env) {
  const matches = projects.filter((candidate) =>
    projectNames.includes(supabaseProjectDisplayName(candidate))
  );
  if (matches.length === 0) return null;
  const orgMatches = matches.filter((candidate) =>
    belongsToOrganization(candidate, organizationId)
  );
  if (orgMatches.length === 1) return orgMatches[0];
  if (orgMatches.length > 1)
    fail(
      `Multiple Supabase projects for ${env} matched ${projectNames.join(", ")} in the configured organization. Set SUPABASE_${env.toUpperCase()}_PROJECT_NAME to disambiguate.`
    );
  if (matches.length === 1) {
    console.log(
      `Supabase project ${supabaseProjectDisplayName(matches[0])} matched by name despite missing/different organization metadata.`
    );
    return matches[0];
  }
  fail(
    `Multiple Supabase projects for ${env} matched ${projectNames.join(", ")}, but none matched the configured organization metadata.`
  );
}
function supabaseProjectNameCandidates(projectSlug, env) {
  return unique([
    firstPresent(
      `SUPABASE_${env.toUpperCase()}_PROJECT_NAME`,
      `SUPABASE_PROJECT_NAME_${env.toUpperCase()}`
    ),
...
async function fetchJson(url, headers, options = {}) {
  ...
  if (!response.ok)
    fail(`Request to ${url} failed with HTTP ${response.status}: ${await response.text()}`);
