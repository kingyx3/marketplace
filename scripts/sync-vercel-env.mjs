...
for (const entry of runtimeEntries) {
  ...
  if (value === undefined || value === "") {
    ...
    if (currentExists) {
      runVercel(["env", "rm", entry.key, vercelEnv, "--yes", "--token", token]);
      removed += 1;
    } else {
      unchanged += 1;
    }
  ...
  if (currentExists) {
    runVercel(
      ["env", "update", entry.key, vercelEnv, "--yes", "--token", token],
      { input: value }
    );
    updated += 1;
  } else {
    runVercel(
      ["env", "add", entry.key, vercelEnv, "--force", "--token", token],
      { input: value }
    );
    added += 1;
  }
...
console.log(
  `Vercel ${vercelEnv} env reconciled: ${added} added, ${updated} updated, ${removed} removed, ${preserved} preserved, ${unchanged} unchanged.`
);
