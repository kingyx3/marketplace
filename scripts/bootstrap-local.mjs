...
checkPrerequisite("Node", process.execPath, ["--version"], (value) => value.startsWith(`v${TOOL_VERSIONS.node.split(".")[0]}.`));
...
const status = run("npx", ["--yes", pinnedNpxPackage("supabase"), "status", "--output", "env"], { capture: true });
const supabase = parseShellEnv(status.stdout);
const envPath = ".env.local";
const current = await readOptionalDotenv(envPath);
const merged = { ...current };
await applyVersionedEnvironmentConfig(merged, { targetEnv: "development", override: false });

setIfPresent(merged, "NEXT_PUBLIC_SUPABASE_URL", supabase.API_URL || supabase.SUPABASE_URL);
setIfPresent(merged, "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY", supabase.ANON_KEY || supabase.PUBLISHABLE_KEY);
...
for (const key of ["NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY", "STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET"]) {
...
const validation = validateEnv({ ...merged, TARGET_ENV: "development", GOOGLE_AUTH_ENABLED: "false" });
if (validation.ok) {
  console.log("Local runtime environment is complete. Run npm run dev.");
} else {
  console.log("Local Supabase and database bootstrap completed. Runtime provider values still required:");
  for (const error of validation.errors) console.log(`  - ${error}`);
  console.log("Add Stripe test values to .env.local, then run npm run env:check.");
}
...
async function readOptionalDotenv(path) {
  try { return parseDotenv(await readFile(path, "utf8")); }
  catch (error) { if (error?.code === "ENOENT") return {}; throw error; }
}
function setIfPresent(target, key, value) { if (value) target[key] = value; }
function checkPrerequisite(name, command, commandArgs, validate, options = {}) {
  const result = spawnSync(command, commandArgs, { encoding: "utf8", stdio: options.quiet ? ["ignore", "pipe", "pipe"] : ["ignore", "pipe", "pipe"] });
  if (result.error || result.status !== 0) fail(`${name} is unavailable. Install/configure it before bootstrapping.`);
...
function run(command, commandArgs, options = {}) {
  ...
  if (result.status !== 0) fail(`${command} failed with exit code ${result.status}${result.stderr ? `\n${result.stderr}` : ""}`);
  return result;
}
