-- Product display names are explicitly managed by admins; URL slugs are derived from them.
-- Category, set, type, and language remain structured product attributes and no longer rename products.
...
alter table public.products
  drop constraint if exists products_name_nonempty;
alter table public.products
  add constraint products_name_nonempty
...
as $$
declare
  v_name text := trim(coalesce(new.name, ''));
  v_slug text;
...
begin
  ...
  if v_slug = '' then
    raise exception 'product display name must generate a valid slug' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where product.slug = v_slug
      and product.id is distinct from new.id
  ) then
    raise exception 'product display name generates a slug already used by another product'
      using errcode = '23505';
  end if;
...
-- Parent taxonomy edits must not silently rename or re-slug products.
...
drop function if exists public.refresh_catalog_product_identity_from_parent();

-- Re-normalize development data under the new contract. Invalid or duplicate rows fail fast.
update public.products
set name = trim(name),
    updated_at = now();

-- Replace the hierarchy creation RPC with a display-name-aware contract.
...
begin
  ...
  if char_length(v_product_name) < 2 or char_length(v_product_name) > 160 then
    raise exception 'product display name must be 2-160 characters' using errcode = '22023';
  end if;

  v_product_slug := public.catalog_slug_from_name(v_product_name);
  if v_product_slug = '' then
    raise exception 'product display name must generate a valid slug' using errcode = '22023';
  end if;

  if exists (
    select 1 from public.products product where product.slug = v_product_slug
  ) then
    raise exception 'product display name generates a slug already used by another product'
      using errcode = '23505';
  end if;
...
-- Replace the product upsert RPC so display-name edits regenerate the slug atomically.
...
revoke all on function public.admin_upsert_catalog_product(
  uuid, text, uuid, uuid, text, text, text, text, boolean, text
) from public, anon, authenticated;
