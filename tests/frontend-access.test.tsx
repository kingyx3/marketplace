...
describe("audience-aware frontend access", () => {
  ...
  it("keeps critical storefront and control surfaces mobile safe", async () => {
    ...
    expect(siteHeader).toContain('import { createPortal } from "react-dom"');
    expect(siteHeader).toContain("createPortal(");
    expect(siteHeader).toContain('role="dialog"');
    expect(siteHeader).toContain('aria-modal="true"');
    expect(siteHeader).toContain("fixed inset-0 z-[100]");
    expect(siteHeader).toContain("bg-zinc-950/80");
    expect(siteHeader).toContain("h-dvh");
    expect(siteHeader).not.toContain("bg-zinc-950/45");
    ...
    expect(controlMobileNavigation).toContain('import { createPortal } from "react-dom"');
    expect(controlMobileNavigation).toContain("createPortal(");
    expect(controlMobileNavigation).toContain('role="dialog"');
    expect(controlMobileNavigation).toContain('aria-modal="true"');
    expect(controlMobileNavigation).toContain("fixed inset-0 z-[100]");
    expect(controlMobileNavigation).toContain("bg-black/80");
    expect(controlMobileNavigation).toContain("h-dvh");
    expect(controlMobileNavigation).not.toContain("bg-black/60");
