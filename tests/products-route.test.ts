...
describe("products storefront route", () => {
  it("ships products pages without legacy catalog route entries", async () => {
    await expect(access(new URL("../app/(shop)/products/page.tsx", import.meta.url))).resolves.toBeUndefined();
    await expect(
      access(new URL("../app/(shop)/products/[slug]/page.tsx", import.meta.url))
    ).resolves.toBeUndefined();
    await expect(
      access(new URL("../app/(shop)/catalog/page.tsx", import.meta.url))
    ).rejects.toBeDefined();
    await expect(
      access(new URL("../app/(shop)/catalog/[slug]/page.tsx", import.meta.url))
    ).rejects.toBeDefined();
  });
  ...
  it("does not reference the retired storefront route from application code", async () => {
    ...
    for (const root of SOURCE_ROOTS) {
      for (const path of await walk(join(ROOT.pathname, root))) {
