...
describe("environment contract", () => {
  ...
  it("requires the HitPay webhook salt because it cannot be generated by bootstrap", () => {
    const withoutWebhook = { ...validEnv };
    delete withoutWebhook.HITPAY_WEBHOOK_SALT;
    const strict = runGenerateEnv<{ ok: boolean; errors: string[] }>(
      `console.log(JSON.stringify(validateEnv(${literal(withoutWebhook)})));`
    );
    const preProvision = runGenerateEnv<{ ok: boolean; errors: string[] }>(
      `console.log(JSON.stringify(validateEnv(${literal(withoutWebhook)}, { allowMissingProvisioned: true })));`
    );
    expect(strict.ok).toBe(false);
    expect(preProvision.ok).toBe(false);
    expect(preProvision.errors.join("\n")).toContain("HITPAY_WEBHOOK_SALT");
  });
  ...
  it("resolves Terraform outputs without overriding explicit environment values", () => {
    ...
    expect(resolved.HITPAY_API_URL).toBe("https://api.sandbox.hit-pay.com");
    expect(resolved.HITPAY_PAYMENT_METHODS).toBe("paynow_online");
  });
    ...
    const resolved = runResolveEnvironment<{
      ...
      for (const target of Object.keys(refs)) {
        ...
        environments[target] = {
          ...
          hitpayApiUrl: env.HITPAY_API_URL,
        };
      }
