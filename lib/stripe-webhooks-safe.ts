...
async function handlePaymentIntentFailed(
  ...
  const changed = await updatePayment(
    supabase,
    payment.id,
    { status: "failed" },
    ["pending", "requires_capture", "authorized"]
  );
  if (!changed || !payment.order_id) return;
...
async function markPreorderBalancePaidFromIntent(
  ...
  if (error) throw new Error(error.message);
  if (typeof data !== "string") throw new Error("preorder conversion did not return an order id");
  return data;
}
  ...
  if (charge.refunded) {
    ...
    if (payment.preorder_id) {
      await supabase
        ...
        .update({ status: "refunded" })
        .eq("id", payment.preorder_id)
        .in("status", ["deposited", "paid", "converted"]);
    }
  }
}
